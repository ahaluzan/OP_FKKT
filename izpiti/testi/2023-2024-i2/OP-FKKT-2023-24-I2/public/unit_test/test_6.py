...
data_in = [('Pablo Escobar: The Drug Lord', 2012, 44, 10, 'crime', 'CO', 8.4, 'TV-14'),
...
actual = naloge.povprecje(data_in, 2001)
expected = False
...
for (exp_key, exp_val), (act_key, act_val) in zip(expected.items(), actual.items()):
    test_case.assertEqual(type(act_key), type(exp_key), "Napacen tip kljuca.")
    test_case.assertEqual(type(act_val), type(exp_val), "Napacen tip vrednosti.")
'''
