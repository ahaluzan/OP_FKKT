...
def run_tests(
    assignment, to_test, tests_root_dir, test_data, dir_names, default_timeout
):
    ...
    while os.path.isfile(
        tests_root + "/" + dir_names["unitTestDir"] + "/test_" + str(test_no) + ".py"
    ):
        ...
        unittest_file = (
            tests_root + "/" + dir_names["unitTestDir"] + "/test_" + str(test_no)
        )
        ...
        unittest_module_code = ast.parse(
            open(unittest_file + ".py", encoding="utf-8").read()
        )
    ...
    with open(config_file) as json_data:
        ...
        for assignment_data in assignments_to_test:
            ...
            if os.path.isfile(test_wrapper):
                print("Public tests:")
                run_tests(
                    assignment_data["assignmentName"],
                    test_wrapper,
                    tests_root_dir,
                    assignment_data["publicTests"],
                    data["testDirNames"],
                    float(data["defaultTimeout"]),
                )
                # print('Private tests:')
